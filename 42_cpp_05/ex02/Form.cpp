...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/12 16:19:45 by jmartini          #+#    #+#             */
/*   Updated: 2021/12/12 16:19:46 by jmartini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string const & name, std::string const & target, const int signGrade, const int execGrade)
...
std::string
Form::getTarget() const { return this->_target ; }
...
Form::execute(Bureaucrat const & executor) const {
	...
	if (this->getSigned() == false) throw SignedException();
	if (executor.getGrade() > this->_execGrade) throw GradeTooHighException();
}
...
operator<<(std::ostream& o, Form const & i) {
	...
	<< "Form target:\t" << i.getTarget() << std::endl
